...
export function useEditableWorkflow({
    ...
    const addStep = useCallback(
        (step: WorkflowStep, afterStepId?: string) => {
            ...
            const initialStepId =
                wf.initialStepId ||
                (steps.length === 1 ? step.id : wf.initialStepId);
    ...
    const removeStep = useCallback(
        (stepId: string) => {
            ...
            const removedStep = workingWorkflow.steps.find(
                (s) => s.id === stepId,
            );
    ...
    const connectSteps = useCallback(
        (sourceId: string, targetId: string) => {
            ...
            if (canReach(workingWorkflow.steps, targetId, effectiveSourceId))
                return;
            ...
            if (sourceId.startsWith("__header__")) {
                ...
                const steps = clearIncomingRefs(
                    workingWorkflow.steps,
                    targetId,
                ).map((s) => (s.id === groupId ? setChildRef(s, targetId) : s));
