...
export function ToolCallParams({
    ...
    useEffect(() => {
        ...
        for (const key of Object.keys(newProps)) {
            const existing = step.params.toolInput[key] as
                | Expression
                | undefined;
            if (existing) {
    ...
    const presentKeys = schemaKeys
        ? [
              ...schemaKeys.filter(
                  (k) => requiredKeys.has(k) || k in toolInput,
              ),
        ...
        <div className="space-y-3">
            <div>
                ...
                {availableToolNames.length > 0 ? (
                    <Combobox
                        open={toolComboboxOpen}
                        onOpenChange={setToolComboboxOpen}
                    >
