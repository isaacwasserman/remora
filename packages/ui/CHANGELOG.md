...
- d5ab5ea: Add `@remoraflow/ui/theme-vars.css` export containing CSS custom properties (step tones, status colors, React Flow mappings, syntax highlighting) extracted from `theme.css`, so consumers can import variables independently of the Tailwind theme block.
...
- 2cac3bc: Render error diagnostic borders with a brighter red in light mode.
- 2cc1b15: Copy theme.css as-is instead of running it through PostCSS, since consumers process it with their own Tailwind. Adds a test that verifies every `--rf-*` token has a corresponding `--color-*` bridge entry.
...
- e8b1ce1: Prevent node deletion in the workflow viewer when editing is disabled. React Flow's built-in delete-key handler removed selected nodes even in view mode; the delete key is now disabled and `remove` node changes are ignored unless `isEditing` is true.
- 2494402: Remove styles.css side-effect import and package.json style field now that all styles live in theme.css
- 480ce0a: Restore theme-colored borders for demo toolbar buttons.
- ae8608a: Show an output connection handle for disconnected workflow steps while editing so their control flow can be repaired in the visual editor.
- b774f10: Scope viewer border defaults to prevent host UI overrides.
- 669bb5b: Fix spurious validation errors caused by undefined-valued keys in workflow definitions produced by the editor
...
- 7e90fd4: Render warning diagnostic borders in yellow in light mode.
- Updated dependencies [8c82c1c]
- Updated dependencies [6787798]
- Updated dependencies [1e20130]
- Updated dependencies [6d7242f]
- Updated dependencies [a7bd38a]
- Updated dependencies [3b663c3]
- Updated dependencies [8c82c1c]
- Updated dependencies [2305db8]
- Updated dependencies [d97e5e4]
- Updated dependencies [3824523]
