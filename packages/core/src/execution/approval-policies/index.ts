...
import { ApprovalPolicyDeniedError } from "./errors";
import type { ApprovalPolicy, FinalApprovalPolicyDecision } from "./types";
import type { PendingApproval } from "../step-executors/llm";
...
export async function* resolveApprovalRequests({
    ...
    StepExecutionUpdate,
    Array<{ type: "tool-approval-response"; approvalId: string; approved: boolean; reason?: string }>
> {
    const asked: Array<{ approval: PendingApproval; interventionId: string }> = [];
    ...
    for (const [i, approval] of approvals.entries()) {
        ...
        await executionContext.step(
            ...
            async () => {
                ...
                if (result.error) {
                    throw new Error(`Could not request approval: ${result.message}`);
                }
            },
        );
    ...
    const responseParts: Array<{ type: "tool-approval-response"; approvalId: string; approved: boolean; reason?: string }> = [];

    for (const [i, { approval, interventionId }] of asked.entries()) {
        const { answer } = yield* executionContext.waitFor(
            [...basePath, "approval", String(i)],
            async () => {
                const received = await userInterventionContext.getResponse(interventionId);
                if (received.error) {
                    throw new Error(`Could not read approval response: ${received.message}`);
                }
                return received.data;
            },
        );
