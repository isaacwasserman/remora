...
describe("sanitizeSchemaForAI", () => {
	...
	test("recurses into nested objects and arrays", () => {
		...
		expect((items.items as Record<string, unknown>).minimum).toBeUndefined();
	});
});
...
describe("createTokenLimitMiddleware", () => {
	test("rejects invalid maxInputTokens", () => {
		expect(() => createTokenLimitMiddleware({ maxInputTokens: 0 })).toThrow();
		expect(() => createTokenLimitMiddleware({ maxInputTokens: -1 })).toThrow();
	});

	test("rejects invalid reductionFactor", () => {
		expect(() =>
			createTokenLimitMiddleware({ maxInputTokens: 100, reductionFactor: 0 }),
		).toThrow();
		expect(() =>
			createTokenLimitMiddleware({ maxInputTokens: 100, reductionFactor: 1 }),
		).toThrow();
	});
	...
	test("proactive truncation removes earlier messages first", async () => {
		...
		const prompt = [
			{ role: "user" as const, content: [{ type: "text" as const, text: longText }] },
			{ role: "assistant" as const, content: [{ type: "text" as const, text: "short" }] },
		];
		...
		const last = output[output.length - 1] as { content: { text: string }[] };
