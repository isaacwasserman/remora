...
import { createCheckpointingExecutionEngine } from ".";
import { testingOnly_createInMemoryCheckpointStore } from "./in-memory-store";

/**
 * Driven through the in-memory checkpoint store, whose map lives as long as the
...
describe("checkpointing execution engine", () => {
    test("getExecutionInfo returns the provided keys", () => {
        const run = engine().createRun("run-1");
    ...
    test("an omitted runId is defaulted to a fresh id per run", () => {
        const shared = engine();
        const first = shared.createRun().getExecutionInfo().runId;
        const second = shared.createRun().getExecutionInfo().runId;
    ...
    test("step result is checkpointed and replayed on a resumed run", async () => {
        const shared = engine();
        let calls = 0;
        const first = await shared.createRun("r").step("s", async () => {
            calls++;
            return calls;
        });
        // A new run bound to the same engine (store) + runId replays.
        const second = await shared.createRun("r").step("s", async () => {
            calls++;
            return calls;
        });
        expect(calls).toBe(1);
    ...
    test("steps with distinct names get distinct checkpoints", async () => {
        ...
        const resumed = shared.createRun("r");
