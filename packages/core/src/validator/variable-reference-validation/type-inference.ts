...
import jmespath from "../../../external/jmespath";
import type { ExpressionNode, FunctionNode } from "../../../external/jmespath/AST.type";
 ...
 * valid JSON Schema. The recursive members are narrowed so nested schemas carry
 ...
 *
 * The JMESPath expression is parsed with the vendored `jmespath.compile`; the
 * resulting AST is interpreted at the type level, mirroring the evaluation
 * semantics of the vendored `TreeInterpreter`.
 */
    ...
    const inferNode = (
        ...
        switch (node.type) {
            ...
            case "Pipe":
                return inferNode(
                    node.right,
                    asSchemaObject(inferNode(node.left, schema)),
                );
            case "Projection": {
                const base = asSchemaObject(inferNode(node.left, schema));
                if (isArraySchema(base)) {
                    return {
                        type: "array",
                        items: inferNode(node.right, arrayElement(base)),
                        badAccess: "false",
                    };
                }
            ...
            case "ValueProjection": {
                ...
                if (isObjectSchema(base)) {
                    return {
                        type: "array",
                        items: inferNode(node.right, objectValueUnion(base)),
                        badAccess: "false",
                    };
                }
                return badNull();
            ...
            case "Flatten": {
                const base = asSchemaObject(inferNode(node.child, schema));
                if (!isArraySchema(base)) {
                    return badNull();
                }
                const element = arrayElement(base);
                // flatten one level: an array-of-arrays collapses to the inner element type
            ...
            case "Root":
                return ensureBadAccess(root, "false");
            case "Literal":
                return {
                    ...asSchemaObject(inferRemoraflowType(node.value)),
                    badAccess: "false",
                };
            case "Comparator":
            case "NotExpression":
            ...
            case "OrExpression":
                return ensureBadAccess(
                    unionSchemas([
                        inferNode(node.left, schema),
                        inferNode(node.right, schema),
                    ]),
                    "false",
                );
            case "Ternary":
                return ensureBadAccess(
                    unionSchemas([
                        inferNode(node.trueExpr, schema),
                        inferNode(node.falseExpr, schema),
                    ]),
                    "false",
                );
            case "Arithmetic":
            case "Unary":
                return { type: "number", badAccess: "false" };
            case "Function":
                return ensureBadAccess(inferFunction(node, schema), "false");
            case "LetExpression":
                return ensureBadAccess(
                    inferNode(node.expression, schema),
                    "false",
                );
            case "Binding":
                return ensureBadAccess(
                    inferNode(node.reference, schema),
                    "false",
                );
            default:
                // Variable, ExpressionReference and anything unhandled resolve
                // to an unknown type. No scope is available to resolve variables.
...
// ---------------------------------------------------------------------------
// Access rules for a single, concrete schema
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Normalization: unknown / union schemas
// ---------------------------------------------------------------------------
...
function accessWithNormalization(
    ...
    if (members) {
        return { anyOf: members.map(single), badAccess: "maybe" };
    }
    return single(s);
}
...
function asSchemaObject(
    definition: RemoraflowType | undefined,
): AnnotatedSchema {
    if (
...
export function unionSchemas(schemas: RemoraflowType[]): AnnotatedSchema {
