...
import { inferSchema, summarizeObjectStructure } from "./summarize";
...
describe("inferSchema", () => {
    ...
    test("dictionary detection with many uniform keys", () => {
        ...
        // The representative value schema keeps the literals of the first entry.
        expect(inferSchema(dict)).toEqual({
            __dict: [
                { name: { __literal: "User 0" }, score: { __literal: 0 } },
                10,
            ],
        });
    });
...
describe("summarizeObjectStructure", () => {
    ...
    test("indent width controls the leading whitespace per level", () => {
        expect(summarizeObjectStructure({ a: 1, b: "hello" }, 2)).toBe(
            '{\n  a: 1,\n  b: "hello"\n}',
        );
        expect(summarizeObjectStructure({ a: 1, b: "hello" }, 4)).toBe(
            '{\n    a: 1,\n    b: "hello"\n}',
        );
    });
    ...
    test("nested structure with indent", () => {
        ...
        expect(summarizeObjectStructure(data, 2)).toBe(
            "{\n  users: {\n    name: string,\n    age: number\n  }[2],\n  metadata: {\n    total: 2\n  }\n}",
        );
    });
});
