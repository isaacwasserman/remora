 ...
 * ignores differences like string[1] vs string[2].
 */
        ...
        if (Array.isArray(value)) {
            ...
            if (totalKeys >= 3) {
                ...
                const normalizedSchemas =
                    valueSchemas.map(normalizeSchemaShape);
                ...
                if (
                    ...
                    if (uniformCount >= 3 && percentUniform >= 0.75) {
                        const representativeSchema = valueSchemas.find((s) =>
                            deepEqual(
                                normalizeSchemaShape(s),
                                mostCommonNormalized,
                            ),
                        );
                        schema = {
    ...
    function stringify(value: unknown, depth = 0): string {
        ...
        if (typeof value === "object") {
            ...
            const formatted = entries
                .map(
                    ([key, val]) =>
                        `${nextIndent}${key}: ${stringify(val, depth + 1)}`,
                )
                .join(",\n");
            return `{\n${formatted}\n${currentIndent}}`;
        }
