...
export const findFirst = (
    ...
    start = Math.max(ensureInteger((start = start || 0)), 0);
        ...
        ensureInteger((end = end || subject.length)),
        subject.length,
    );
...
const ensurePadFuncParams = (
    ...
    if (padding.length > 1) {
        throw new Error(
            `invalid value, ${name} expects its 'pad' parameter to be a valid string with a single codepoint`,
        );
    }
...
export const padLeft = (
    ...
    return subject?.padStart(width, padding) || "";
};
export const padRight = (
    ...
    return subject?.padEnd(width, padding) || "";
};
