...
import type { ScopeEntry } from "./Parser.type";
...
export class TreeInterpreter {
    ...
    visit(
        node: ExpressionNode,
        value: JSONValue | ExpressionNode,
    ): JSONValue | ExpressionNode | ExpressionReference {
        switch (node.type) {
            case "Ternary": {
            ...
            case "Variable": {
                ...
                if (
                    ...
                    !Object.hasOwn(this._scope.currentScopeData, variable)
                ) {
                    throw new Error(
                        `Error referencing undefined variable ${variable}`,
                    );
                }
            ...
            case "Subexpression": {
                ...
                return result != null
                    ? (this.visit(node.right, result) ?? null)
                    : null;
            }
            case "Index": {
                ...
                const index =
                    node.value < 0 ? value.length + node.value : node.value;
            ...
            case "Slice": {
                ...
                const { start, stop, step } = this.computeSliceParams(
                    value.length,
                    node,
                );
            ...
            case "Arithmetic": {
                ...
                switch (node.operator) {
                    ...
                    default:
                        throw new Error(
                            `Syntax error: unknown arithmetic operator: ${node.operator}`,
                        );
                }
            }
            case "Unary": {
        ...
        start =
            start === null
                ? step < 0
                    ? arrayLength - 1
                    : 0
                : this.capSliceRange(arrayLength, start, step);
        stop =
            stop === null
                ? step < 0
                    ? -1
                    : arrayLength
                : this.capSliceRange(arrayLength, stop, step);
