...
import { type LexerToken, Token } from "./Lexer.type";
import type { Options } from "./Parser.type";
...
class TokenParser {
    ...
    parse(expression: string, options?: Options): ExpressionNode {
        this.loadTokens(
            expression,
            options || { enable_legacy_literals: false },
        );
        this.index = 0;
        ...
        if (this.lookahead(0) !== Token.TOK_EOF) {
            ...
            this.errorToken(
                token,
                `Syntax error: unexpected token type: ${token.type}, value: ${token.value}`,
            );
        }
        return ast;
    }
    ...
    expression(rbp: number): ExpressionNode {
        ...
        while (rbp < (bindingPower[currentTokenType] as number)) {
            this.advance();
    ...
    private lookahead(offset: number): Token {
        return (this.tokens[this.index + offset] as LexerToken).type;
    }

    private lookaheadToken(offset: number): LexerToken {
        return this.tokens[this.index + offset] as LexerToken;
    }
    ...
    nud(token: LexerToken): ExpressionNode {
        switch (token.type) {
            ...
            case Token.TOK_UNQUOTEDIDENTIFIER: {
                if (
                    TokenParser.isKeyword(token, "let") &&
                    this.lookahead(0) === Token.TOK_VARIABLE
                ) {
            ...
            case Token.TOK_QUOTEDIDENTIFIER:
                if (this.lookahead(0) === Token.TOK_LPAREN) {
                    throw new Error(
                        "Syntax error: quoted identifier not allowed for function names.",
                    );
                } else {
                    return { type: "Field", name: token.value as string };
                }
            case Token.TOK_NOT: {
                const child = this.expression(bindingPower.Not as number);
                return { type: "NotExpression", child };
            }
            case Token.TOK_MINUS: {
                const child = this.expression(bindingPower.Minus as number);
                return {
                    type: "Unary",
                    operator: token.type,
                    operand: child,
            ...
            case Token.TOK_PLUS: {
                const child = this.expression(bindingPower.Plus as number);
                return {
                    type: "Unary",
                    operator: token.type,
                    operand: child,
            ...
            case Token.TOK_STAR: {
                ...
                return {
                    ...
                    left,
                    right: this.parseProjectionRHS(bindingPower.Star as number),
                };
            }
            ...
            case Token.TOK_FLATTEN: {
                ...
                const right: ExpressionNode = this.parseProjectionRHS(
                    bindingPower.Flatten as number,
                );
                return { type: "Projection", left, right };
            }
            case Token.TOK_LBRACKET: {
                if (
                    this.lookahead(0) === Token.TOK_NUMBER ||
                    this.lookahead(0) === Token.TOK_COLON
                ) {
                    const right = this.parseIndexExpression();
                    return this.projectIfSlice({ type: "Identity" }, right);
                }
                if (
                    this.lookahead(0) === Token.TOK_STAR &&
                    this.lookahead(1) === Token.TOK_RBRACKET
                ) {
                    this.advance();
                    this.advance();
                    const right = this.parseProjectionRHS(bindingPower.Star as number);
                    return {
                        left: { type: "Identity" },
                        right,
                        type: "Projection",
            ...
            case Token.TOK_EXPREF: {
                const child = this.expression(bindingPower.Expref as number);
        ...
        switch (tokenName) {
            ...
            case Token.TOK_DOT: {
                ...
                if (this.lookahead(0) !== Token.TOK_STAR) {
                    const right = this.parseDotRHS(rbp as number);
                    return { type: "Subexpression", left, right };
                }
                this.advance();
                const right = this.parseProjectionRHS(rbp as number);
                return { type: "ValueProjection", left, right };
            }
            case Token.TOK_PIPE: {
                const right = this.expression(bindingPower.Pipe as number);
                return { type: "Pipe", left, right };
            }
            case Token.TOK_OR: {
                const right = this.expression(bindingPower.Or as number);
                return { type: "OrExpression", left, right };
            }
            case Token.TOK_AND: {
                const right = this.expression(bindingPower.And as number);
            ...
            case Token.TOK_LPAREN: {
                ...
                const args = this.parseCommaSeparatedExpressionsUntilToken(
                    Token.TOK_RPAREN,
                );
            ...
            case Token.TOK_FILTER: {
                ...
                const right: ExpressionNode =
                    this.lookahead(0) === Token.TOK_FLATTEN
                        ? { type: "Identity" }
                        : this.parseProjectionRHS(bindingPower.Filter as number);
                return { type: "FilterProjection", left, right, condition };
            }
            case Token.TOK_FLATTEN: {
                const leftNode: UnaryExpressionNode = {
                    type: "Flatten",
                    child: left,
                };
                const right = this.parseProjectionRHS(bindingPower.Flatten as number);
            ...
            case Token.TOK_LBRACKET: {
                const token = this.lookaheadToken(0);
                if (
                    token.type === Token.TOK_NUMBER ||
                    token.type === Token.TOK_COLON
                ) {
                    const right = this.parseIndexExpression();
    ...
    private static isKeyword(token: LexerToken, keyword: string): boolean {
        return (
            token.type === Token.TOK_UNQUOTEDIDENTIFIER &&
            token.value === keyword
        );
    }
    ...
    private match(tokenType: Token | LexerToken): void {
        if (this.lookahead(0) === tokenType) {
            ...
            this.errorToken(
                token,
                `Syntax error: expected ${tokenType}, got: ${token.type}`,
            );
        }
    }
    ...
    private errorToken(token: LexerToken, message = ""): never {
        const error = new Error(
            message ||
                `Syntax error: invalid token (${token.type}): "${token.value}"`,
        );
    ...
    private parseIndexExpression(): SliceNode | IndexNode {
        if (
            this.lookahead(0) === Token.TOK_COLON ||
            this.lookahead(1) === Token.TOK_COLON
        ) {
    ...
    private parseSliceExpression(): SliceNode {
            ...
            if (current.type === Token.TOK_COLON) {
                ...
                if (index === 3) {
                    this.errorToken(
                        ...
                        "Syntax error, too many colons in slice expression",
                    );
                }
                this.advance();
                ...
                this.errorToken(
                    next,
                    `Syntax error, unexpected token: ${next.value}(${next.type})`,
                );
            }
        ...
        return { type: "Slice", start: start as number, stop: stop as number, step: step as number };
    }
    ...
    private parseComparator(
        ...
        const right = this.expression(bindingPower[comparator] as number);
    ...
    private parseArithmetic(
        ...
        const right = this.expression(bindingPower[operator] as number);
    ...
    private parseDotRHS(rbp: number): ExpressionNode {
        ...
        const exprTokens = [
            Token.TOK_UNQUOTEDIDENTIFIER,
            Token.TOK_QUOTEDIDENTIFIER,
            Token.TOK_STAR,
        ];
        ...
        this.errorToken(
            token,
            `Syntax error, unexpected token: ${token.value}(${token.type})`,
        );
    }

    private parseProjectionRHS(rbp: number): ExpressionNode {
        if (bindingPower[this.lookahead(0)] as number < 10) {
    ...
    private parseMultiselectHash(): ExpressionNode {
        ...
        for (;;) {
            ...
            if (!identifierTypes.includes(keyToken.type)) {
                throw new Error(
                    `Syntax error: expecting an identifier token, got: ${keyToken.type}`,
                );
            }
