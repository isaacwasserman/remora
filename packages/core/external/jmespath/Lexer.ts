...
import { type LexerOptions, type LexerToken, Token } from "./Lexer.type";
...
class StreamLexer {
    ...
    tokenize(stream: string, options?: LexerOptions): LexerToken[] {
        ...
        while (this._current < stream.length) {
            if (isAlpha(stream[this._current] as string)) {
                start = this._current;
            ...
            } else if (basicTokens[stream[this._current] as string] !== undefined) {
                tokens.push({
                    start: this._current,
                    type: basicTokens[stream[this._current] as string] as Token,
                    value: stream[this._current] as JSONValue,
                });
                this._current += 1;
            } else if (stream[this._current] === "$") {
                start = this._current;
                if (
                    this._current + 1 < stream.length &&
                    isAlpha(stream[this._current + 1] as string)
                ) {
                    this._current += 1;
                    identifier = this.consumeUnquotedIdentifier(stream);
                    tokens.push({
                ...
                if (
                    this._current + 1 < stream.length &&
                    isNum(stream[this._current + 1] as string)
                ) {
            ...
            } else if (isNum(stream[this._current] as string)) {
                ...
                operatorStartToken[stream[this._current] as string] !== undefined
            ) {
                token = this.consumeOperator(stream) as LexerToken;
                token && tokens.push(token);
            } else if (skipChars[stream[this._current] as string] !== undefined) {
                this._current += 1;
            } else {
                const error = new Error(
                    `Syntax error: unknown character: ${stream[this._current]}`,
                );
    ...
    private consumeUnquotedIdentifier(stream: string): string {
        ...
        while (
            ...
            isAlphaNum(stream[this._current] as string)
        ) {
            this._current += 1;
        }
    ...
    private consumeNumber(stream: string): LexerToken {
        ...
        this._current += 1;
        const maxLength = stream.length;
        while (isNum(stream[this._current] as string) && this._current < maxLength) {
            this._current += 1;
        }
    ...
    private consumeOrElse(
        ...
        return { start: start, type: token, value: stream[start] as string };
    }

    private consumeOperator(stream: string): LexerToken | undefined {
        const start = this._current;
        const startingChar = stream[start];
        switch (startingChar) {
            case "!":
                return this.consumeOrElse(
                    stream,
                    "=",
                    Token.TOK_NOT,
                    Token.TOK_NE,
    ...
    private looksLikeJSON(literalString: string): boolean {
        ...
        if (startingChars.includes(literalString[0] as string)) {
            return true;
        }
        if (jsonLiterals.includes(literalString)) {
            return true;
        }
        if (numberLooking.includes(literalString[0] as string)) {
