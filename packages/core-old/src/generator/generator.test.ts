...
function createMockModel(workflows: WorkflowDefinition[]) {
    ...
    return new MockLanguageModelV3({
        doGenerate: async () => {
            ...
            return mockResult(
                { toolName: "createWorkflow", input: workflow },
                i,
            );
        },
    });
...
describe("generateWorkflow", () => {
    ...
    test("giveUp: agent retries createWorkflow then gives up", async () => {
        const model = createMockModelWithCalls([
            { toolName: "createWorkflow", input: invalidWorkflow },
            {
                ...
                input: {
                    code: "infeasible",
                    reason: "Required tool is missing.",
                },
            },
        ]);
    ...
    test("giveUp: tool input schema exposes per-code descriptions", async () => {
                ...
                return mockResult(
                    {
                        ...
                        input: {
                            code: "other",
                            reason: "stopping early to inspect tools",
                        },
                    },
                    0,
                );
        ...
        const variants = (
            codeSchema as {
                anyOf: Array<{ const: string; description: string }>;
            }
        ).anyOf;
