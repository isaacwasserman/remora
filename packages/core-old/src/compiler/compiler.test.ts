...
const ticketReviewWorkflow = {
    ...
    steps: [
        ...
        },
        {
            ...
            type: "llm-prompt",
            params: {
                prompt: "Classify this support ticket as critical or routine.\n\nSubject: ${ticket.subject}\nBody: ${ticket.body}",
                outputFormat: {
                    type: "object",
                    properties: {
                        classification: {
...
describe("integration: real workflow files", () => {
    ...
    test("headlines workflow produces INVALID_STEP_ID for hyphenated IDs", async () => {
        ...
        const invalidIds = getDiagnostics(
            result.diagnostics,
            "INVALID_STEP_ID",
        );
    ...
    test("order-fulfillment workflow compiles with zero errors", async () => {
        ...
        const result = await compileWorkflow(
            task.workflow as WorkflowDefinition,
            {
...
describe("step reference validation", () => {
    ...
    test("missing nextStepId reference", async () => {
        ...
        const result = await compileWorkflow(workflow);
        expect(hasDiagnostic(result.diagnostics, "MISSING_NEXT_STEP")).toBe(
            true,
        );
    ...
    test("missing branchBodyStepId reference", async () => {
        ...
        const result = await compileWorkflow(workflow);
        expect(
            hasDiagnostic(result.diagnostics, "MISSING_BRANCH_BODY_STEP"),
        ).toBe(true);
    });
    ...
    test("missing loopBodyStepId reference", async () => {
        ...
        const result = await compileWorkflow(workflow);
        expect(
            hasDiagnostic(result.diagnostics, "MISSING_LOOP_BODY_STEP"),
        ).toBe(true);
    });
    ...
    test("duplicate step IDs", async () => {
        ...
        const result = await compileWorkflow(workflow);
        expect(hasDiagnostic(result.diagnostics, "DUPLICATE_STEP_ID")).toBe(
            true,
        );
...
describe("graph analysis", () => {
    test("unreachable step", async () => {
        ...
        const result = await compileWorkflow(workflow);
        expect(hasDiagnostic(result.diagnostics, "UNREACHABLE_STEP")).toBe(
            true,
        );
...
describe("control flow validation", () => {
    test("end step with nextStepId", async () => {
        ...
        const result = await compileWorkflow(workflow);
        expect(hasDiagnostic(result.diagnostics, "END_STEP_HAS_NEXT")).toBe(
            true,
        );
    });
    ...
    test("switch-case with multiple default cases", async () => {
        ...
        const result = await compileWorkflow(workflow);
        expect(
            hasDiagnostic(result.diagnostics, "MULTIPLE_DEFAULT_CASES"),
        ).toBe(true);
    });
    ...
    test("loop body that escapes to main flow", async () => {
        ...
        const result = await compileWorkflow(workflow);
        expect(hasDiagnostic(result.diagnostics, "LOOP_BODY_ESCAPES")).toBe(
            true,
        );
    });
    ...
    test("branch body that escapes to main flow", async () => {
        ...
        const result = await compileWorkflow(workflow);
        expect(hasDiagnostic(result.diagnostics, "BRANCH_BODY_ESCAPES")).toBe(
            true,
        );
    });
});
...
describe("tool validation", () => {
    test("unknown tool produces error", async () => {
        ...
        (
            workflow.steps[0] as { params: { toolName: string } }
        ).params.toolName = "nonexistent-tool";
    ...
    test("missing required tool input key produces error", async () => {
        ...
        const result = await compileWorkflow(workflow, { tools: testTools });
        expect(
            hasDiagnostic(result.diagnostics, "MISSING_TOOL_INPUT_KEY"),
        ).toBe(true);
        const diag = getFirstDiagnostic(
            result.diagnostics,
        ...
        const workflow: WorkflowDefinition = {
            ...
            steps: [
                ...
                },
                {
                    ...
                    name: "After Switch",
                    description:
                        "References branch step — may not have executed",
                    type: "tool-call",
                    params: {
                        toolName: "do-thing",
                        toolInput: {
            ...
            steps: [
                {
                    ...
                    name: "Before",
                    description:
                        "Runs before the wait — body has not executed yet",
                    type: "tool-call",
                    params: {
                        toolName: "do-thing",
                        toolInput: {
...
describe("itemName validation", () => {
    test("invalid itemName format produces error", async () => {
        ...
        const result = await compileWorkflow(workflow);
        expect(hasDiagnostic(result.diagnostics, "INVALID_ITEM_NAME")).toBe(
            true,
        );
        const diag = getFirstDiagnostic(
            result.diagnostics,
    ...
    test("itemName that shadows a step ID produces warning", async () => {
        ...
        const result = await compileWorkflow(workflow);
        expect(
            hasDiagnostic(result.diagnostics, "ITEM_NAME_SHADOWS_STEP_ID"),
        ).toBe(true);
        const diag = getFirstDiagnostic(
            result.diagnostics,
        ...
        const workflow: WorkflowDefinition = {
            ...
            steps: [
                ...
                },
                {
                    ...
                    name: "After",
                    description:
                        "After switch — tries to use loop var from branch",
                    type: "tool-call",
                    params: {
                        toolName: "do-thing",
                        toolInput: {
...
describe("best practices", () => {
    ...
    describe("addMissingEndSteps", () => {
        ...
        test("adds end step to a terminal non-end step on the main chain", async () => {
            ...
            const toolStep = result.workflow?.steps.find(
                (s) => s.id === "start",
            );
...
describe("compiler limits", () => {
    ...
    test("timeoutMs exceeding limit errors", async () => {
        const result = await compileWorkflow(
            waitWorkflow({ timeoutMs: 700_000 }),
            {
                tools,
            },
        );
    ...
    test("timeoutMs within limit passes", async () => {
        const result = await compileWorkflow(
            waitWorkflow({ timeoutMs: 500_000 }),
            {
                tools,
            },
        );
    ...
    test("custom backoff range", async () => {
        const result = await compileWorkflow(
            ...
            {
                tools,
                limits: { minBackoffMultiplier: 1, maxBackoffMultiplier: 1.2 },
            },
        );
        expect(
            hasDiagnostic(
            ...
            steps: [
                ...
                },
                {
                    ...
                    params: {
                        output: {
                            type: "jmespath",
                            expression: "analyze.nonexistent",
                        },
                    },
                },
            ] as WorkflowDefinition["steps"],
            ...
            steps: [
                {
                    ...
                    description:
                        "Start the daily vulnerability report workflow",
                },
                {
                    ...
                    params: {
                        ...
                        toolInput: {
                            sql: {
                                type: "literal",
                                value: "SELECT COUNT(*) AS total_new_vulns FROM otto_dev26.otto_vulnerabilities v WHERE v.partition_date = CURRENT_DATE AND v.first_seen_date = CURRENT_DATE",
                            },
                            dataTableContext: {
                                type: "literal",
                ...
                },
                {
                    ...
                    params: {
                        ...
                        toolInput: {
                            sql: {
                                type: "literal",
                                value: "SELECT kb.severity, COUNT(*) AS vuln_count FROM otto_dev26.otto_vulnerabilities v LEFT JOIN otto_dev26.otto_qualys_vuln_kb kb ON CAST(kb.qid AS VARCHAR) = v.qualys_qid WHERE v.partition_date = CURRENT_DATE AND v.first_seen_date = CURRENT_DATE GROUP BY kb.severity ORDER BY vuln_count DESC",
                            },
                            dataTableContext: {
                                type: "literal",
                                value: "New vulnerability counts broken down by severity for today",
                            },
                        },
                    },
                    nextStepId: "query_top_hosts",
                    description:
                        "Count new vulnerabilities grouped by severity",
                },
                {
                    id: "query_top_hosts",
                    ...
                    params: {
                        ...
                        toolInput: {
                            sql: {
                                type: "literal",
                                value: "SELECT v.hostname, COUNT(*) AS new_vuln_count FROM otto_dev26.otto_vulnerabilities v WHERE v.partition_date = CURRENT_DATE AND v.first_seen_date = CURRENT_DATE GROUP BY v.hostname ORDER BY new_vuln_count DESC LIMIT 5",
                            },
                            dataTableContext: {
                                type: "literal",
                ...
                },
                {
                    ...
                    description:
                        "Workflow complete — daily vulnerability report sent",
                },
            ],
        } as WorkflowDefinition;
