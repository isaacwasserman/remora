        ...
        switch (decision.type) {
            ...
            case "request": {
                ...
                if (approvalDecision.approved) {
                    await stateManager?.stepApproved(
                        stepId,
                        execPath,
                        sourcePolicyId,
                    );
        ...
        if (properties && typeof properties === "object") {
            for (const [key, value] of Object.entries(
                ...
                if (
                    ...
                    const propExpectedType = (propSchema as { type: string })
                        .type;
                    const actualType = typeof value;
                    if (
    ...
    switch (step.type) {
        ...
        case "extract-data":
            return executeExtractData(
                ...
                scope,
                options,
                timer.resolvedLimits,
            );
        case "agent-loop":
            return executeAgentLoop(step, scope, options, timer.resolvedLimits);
        case "tool-call":
            return {
                output: await executeToolCall(step, scope, options.tools),
            };
        case "switch-case":
            return {
        ...
        if (step.type === "tool-call" && options.policies?.length) {
            const action: ApprovableAction = {
                ...
                params: {
                    ...
                    toolInput: (resolvedInputs ?? {}) as Record<
                        string,
                        unknown
                    >,
                },
