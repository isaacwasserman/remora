...
test("strips minItems > 1 and maxItems from array schemas", () => {
    ...
    expect(
        (
            (sanitized.properties.friends as Record<string, unknown>)
...
test("preserves minItems of 0 and 1", () => {
    ...
    expect((sanitized.properties.a as Record<string, unknown>).minItems).toBe(
        0,
    );
    expect((sanitized.properties.b as Record<string, unknown>).minItems).toBe(
        1,
    );
});
