...
describe("Approval request flow", () => {
            ...
            policies: [
                {
                    ...
                    decider: async () => ({
                        ...
                        staleFn: () => {
                            ...
                            return {
                                stale: true,
                                reason: "Workflow re-run detected",
                            };
                        },
                        conditionFn: () => {
    ...
    test("backoff increases polling interval over time", async () => {
        ...
        if (pollTimes.length >= 4) {
            const interval1 =
                (pollTimes[2] as number) - (pollTimes[1] as number);
            const interval2 =
                (pollTimes[3] as number) - (pollTimes[2] as number);
