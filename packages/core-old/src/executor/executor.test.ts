                    ...
                    params: {
                        ...
                        toolInput: {
                            value: {
                                type: "template",
                                template:
                                    "${fetch.items[0].name}-${fetch.items[1].name}",
                            },
                        },
                    },
                }),
        ...
        const workflow: WorkflowDefinition = {
            ...
            steps: [
                ...
                step("agent_step", {
                    ...
                    params: {
                        instructions:
                            "Process the greeting: ${get_data.greeting}",
                        tools: ["echo"],
                        outputFormat: {
                            type: "object",
                            properties: { result: { type: "string" } },
...
describe("extract-data probe mode", () => {
    ...
    test("inline mode give-up throws ExtractionError without retrying", async () => {
        const workflow = makeExtractWorkflow(
                    ...
                    content: [
                        {
                            ...
                            toolName: "give-up",
                            input: JSON.stringify({
                                reason: "The source data contains weather information, not user data",
                            }),
                        },
                    ],
                    finishReason: {
...
describe("resumption via initialState", () => {
    ...
    function makeInitialState(
        completedSteps: { stepId: string; output: unknown }[],
        workflow: WorkflowDefinition = threeStepWorkflow,
    ): ExecutionState {
        return {
            runId: "resume-run-id",
            status: "failed",
            startedAt: "2026-01-01T00:00:00.000Z",
            workflowHash: hashWorkflow(workflow),
            stepRecords: completedSteps.map((s) => ({
                stepId: s.stepId,
    ...
    test("resumes from the first incomplete step", async () => {
        ...
        const result = await executeWorkflow(threeStepWorkflow, {
            tools: testTools,
            initialState,
            onStepStart: (stepId) => stepsExecuted.push(stepId),
        });

        expect(result.success).toBe(true);
        // "fetch" should be skipped since it was already completed
        expect(stepsExecuted).not.toContain("fetch");
        expect(stepsExecuted).toContain("process");
        expect(stepsExecuted).toContain("classify");
        // The skipped step's output should still be available
        expect(result.stepOutputs.fetch).toEqual({
            items: [
                { name: "alpha", category: "a" },
                { name: "beta", category: "b" },
                { name: "gamma", category: "a" },
        ...
        expect(result.stepOutputs.process).toEqual({ processed: "ALPHA" });
        expect(result.stepOutputs.classify).toEqual({ label: "long" });
    });

    test("skips all steps when all are already completed", async () => {
        const stepsExecuted: string[] = [];
        const initialState = makeInitialState([
            {
                stepId: "fetch",
            ...
            { stepId: "process", output: { processed: "ALPHA" } },
            { stepId: "classify", output: { label: "long" } },
        ]);
        ...
        expect(result.output).toEqual({ label: "long" });
    });

    test("preserves runId from initial state", async () => {
        const initialState = makeInitialState([
            {
                stepId: "fetch",
                output: {
        ...
        expect(result.success).toBe(true);
        expect(result.executionState.runId).toBe("resume-run-id");
    });

    test("produces same output as a fresh run", async () => {
        const freshResult = await executeWorkflow(threeStepWorkflow, {
            tools: testTools,
        });

        const initialState = makeInitialState([
            { stepId: "fetch", output: freshResult.stepOutputs.fetch },
        ]);

        const resumedResult = await executeWorkflow(threeStepWorkflow, {
            tools: testTools,
            initialState,
        });

        expect(resumedResult.success).toBe(true);
        expect(resumedResult.output).toEqual(freshResult.output);
        expect(resumedResult.stepOutputs.process).toEqual(
            freshResult.stepOutputs.process,
        );
        expect(resumedResult.stepOutputs.classify).toEqual(
            freshResult.stepOutputs.classify,
        );
    });

    test("ignores initialState when workflowHash mismatches", async () => {
        const stepsExecuted: string[] = [];
        // Create initial state with a different workflow's hash
        const differentWorkflow: WorkflowDefinition = {
            initialStepId: "x",
            steps: [
                step("x", {
                    type: "tool-call",
        ...
        const initialState = makeInitialState(
            [
                {
                    stepId: "fetch",
                    output: { items: [{ name: "alpha", category: "a" }] },
                },
            ],
            differentWorkflow,
        );
        ...
        expect(result.success).toBe(true);
        // All steps should execute since the hash didn't match
        expect(stepsExecuted).toContain("fetch");
        expect(stepsExecuted).toContain("process");
        expect(stepsExecuted).toContain("classify");
    });

    test("stamps workflowHash on execution state", async () => {
        const result = await executeWorkflow(threeStepWorkflow, {
            tools: testTools,
        });

        expect(result.success).toBe(true);
        expect(result.executionState.workflowHash).toBe(
            hashWorkflow(threeStepWorkflow),
        );
    });
});
