...
describeE2E("e2e: LLM workflow generation and execution", () => {
    ...
    describe("multi-warehouse order fulfillment", () => {
        ...
        const tools = {
            ...
            "reserve-inventory": tool({
                description:
                    "Reserve inventory for an item at a specific warehouse",
                inputSchema: type({
                    itemId: "string",
                    quantity: "number",
                    warehouse: "string",
        ...
        test("mixed: primary, alternate fallback, and backorder", async () => {
            ...
            expect(
                reservations.some(
                    (r) =>
                        r.itemId === "GADGET-B" && r.warehouse === "alternate",
                ),
            ).toBe(true);
            // DOOHICK-C not reserved at all (backordered)
            expect(reservations.some((r) => r.itemId === "DOOHICK-C")).toBe(
                false,
            );
            // 2 shipments
    ...
    describe("content publishing with quality gates", () => {
        ...
        const tools = {
            ...
            "publish-content": tool({
                ...
                execute: async (input) => {
                    ...
                    return {
                        url: `https://example.com/articles/${input.submissionId}`,
                    };
                },
            }),
            ...
            "request-revision": tool({
                description:
                    "Send a revision request to the author with feedback",
                inputSchema: type({
                    submissionId: "string",
                    feedback: "string",
                }),
            ...
            "archive-content": tool({
                description:
                    "Archive a submission that doesn't meet quality standards",
                inputSchema: type({ submissionId: "string", reason: "string" }),
        ...
        test("mixed: one plagiarized, others go through quality gate", async () => {
            ...
            expect(rejectCalls.some((c) => c.submissionId === "SUB-2")).toBe(
                true,
            );
        ...
        test("single high-quality submission with many platforms", async () => {
            ...
            if (publishCalls.length === 1) {
                ...
                const platforms = new Set(
                    socialPostCalls.map((c) => c.platform),
                );
...
After processing all incidents, send a triage summary to the ops channel.`,
        ...
        test("mixed severity: critical resolved, high and low get tickets", async () => {
            ...
            expect(remediations.some((r) => r.incidentId === "INC-1")).toBe(
                true,
            );
            expect(verifications.some((v) => v.service === "payment-api")).toBe(
                true,
            );
            // Critical incident resolved (verification passed)
            expect(resolutions.some((r) => r.incidentId === "INC-1")).toBe(
                true,
            );
