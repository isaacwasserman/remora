...
function workflowNeedsLLM(wf: WorkflowDefinition): boolean {
    return wf.steps.some((s) =>
        LLM_STEP_TYPES.has((s as { type: string }).type),
    );
}
...
export function App() {
    ...
    const handleGenerate = useCallback(
        async (task: string) => {
            if (!apiKey) {
                setGenerateError(
                    "Please configure an OpenRouter API key in Settings.",
                );
                return;
            }
        ...
        <div className="h-full flex flex-col bg-background text-foreground">
            ...
            <header className="bg-card dark:bg-secondary/40 border-b border-border px-4 py-2.5 flex items-center gap-4 shrink-0">
                ...
                <Button
                    ...
                    onClick={handleReset}
                    disabled={
                        !execution.isRunning && !isDone && !execution.isPaused
                    }
                    className="gap-1.5"
                >
                            ...
                            <div className="space-y-3">
                                ...
                                <label className="block text-xs font-medium text-muted-foreground">
                                    ...
                                    <Input
                                        value={modelId}
                                        onChange={(e) =>
                                            handleSaveModelId(e.target.value)
                                        }
                                ...
                                <p className="text-[10px] text-muted-foreground">
                                    Required for llm-prompt, extract-data, and
                                    agent-loop steps.
                                </p>
                            </div>
                        </div>
