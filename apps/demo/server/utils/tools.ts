...
export const DEMO_TOOLS = {
    ...
    "list-pokemon": tool({
        description:
            "List Pokémon with optional filters. Filter by type (e.g. 'fire') and/or generation (e.g. 1 for Kanto). Returns up to 20 matching Pokémon.",
        inputSchema: type({
            "type?": "string",
            "generation?": "number",
            limit: "number = 20",
        ...
        execute: async ({ type: typeName, generation, limit }) => {
            ...
            if (typeName) {
                ...
                if (generation) {
                    ...
                    const genSpecies = new Set(
                        genData.pokemon_species.map(
                            (s: { name: string }) => s.name,
                        ),
                    );
                    pokemonList = pokemonList.filter((p) =>
                        genSpecies.has(p.name),
                    );
                }
            ...
            const res = await fetch(
                `https://pokeapi.co/api/v2/pokemon?limit=${max}`,
            );
